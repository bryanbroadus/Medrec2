...
<head>
	<meta charset="utf-8">
	<title>MedRec - View Invoices</title>
...
{ 	     jConfirm('Do You Want To Delete This Inovice?', 'Confirmation Dialog', function (r) {
           if(r){ 
               console.log();
                $.ajax({
  			url: "delete.php",
  			...
  			success: function(data) {
    			window.location ='view_sales.php';
                        ...
                        jAlert('Invoice has been Deleted', 'MedRec');
  			}
		});
            }
            return r;
...
return false;
 ...
 jConfirm('Sure You Want To Delete Invoice?', 'Confirmation Dialog', function (r) {
           if(r){ 
...
// -->
		...
		$("#form1").validate({
			...
			messages: {
				name: {
					...
					minlength: "Patient must consist of at least 3 characters"
				},
				address: {
					minlength: "Patient Address must be at least 3 characters long",
					maxlength: "Patient Address must be at least 3 characters long"
				}
			}
		});
	...
	<script>
		...
		{
			newwindow=window.open(url,'name','width=620,height=800,toolbar=0,menubar=no,status=no,resizable=yes,location=no,directories=no');
	...
	<div id="content">
		...
		<div class="page-full-width cf">
			...
			<div class="side-menu fl">
				...
				<h3>Invoices</h3>
				<ul>
					<li><a href="add_sales.php">Add New Invoice</a></li>
...
$SQL = "SELECT DISTINCT(transactionid) FROM  stock_sales WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR customer_id LIKE '%".$_POST['searchtxt']."%' OR transactionid  LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' ";
	...
	if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
	...
	$sql= "SELECT DISTINCT(transactionid) FROM stock_sales WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR customer_id LIKE '%".$_POST['searchtxt']."%' OR transactionid  LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' ORDER BY date desc LIMIT $start, $limit";
;
...
<tbody>
	<tr>
		...
		<td><a href="update_sales.php?sid=<?php echo $line->transactionid;?>&table=stock_sales&return=view_sales.php"><i class="btn btn-primary btn-xs fa fa-pencil"></i></a>
			<a  href="javascript:confirmSubmit(<?php echo $row['transactionid']?>,'stock_sales','view_sales.php')"><i class="btn btn-danger btn-xs fa fa-trash-o"></i></a></td>
		<td><a href="invoice_print.php?sid=<?php echo $row['transactionid'];?>" onClick="poptastic(this.href); return false;"><i class="fa btn btn-success btn-xs fa-print fa-1x"></i></a></td>
		<td><input type="checkbox" value="<?php echo $row['id']; ?>" name="checklist[]" id="check_box" /></td>
	</tr>
<?php $i++; } ?>
	<tr>
       <td align="center"><div style="margin-left:0px;"><?php echo $pagination; ?></div></td>
    </tr>
	  </tbody>
</table>
</form>
